...
function Categorypage() {
  ...
  return (
   <div className='w-full min-h-screen p-3 '>
    ...
    <div className='w-full h-auto grid md:grid-cols-4 grid-cols-2 gap-3 py-2 px-3 md:px-0 capitalize font-bold  '>
      {category.map((item,id)=>{
        return(
       <Link key={id} to={`/geners/${item}`}> <div  className='category h-24 md:h-44 rounded-lg md:cursor-pointer md:text-2xl text-sm flex items-center px-2'>{item} </div></Link>  
        )
      })}
